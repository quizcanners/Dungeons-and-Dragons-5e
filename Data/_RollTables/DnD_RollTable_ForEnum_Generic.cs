...
namespace Dungeons_and_Dragons.Tables
{
    ...
    {
        internal const string SO_MENU_NAME_CONCEPT = "Enum Concept/";
        ...
        [Serializable]
        public class Element : RollTableElementWithSubTablesBase
        {
            public T Value;
            ...
            public override void InspectInList(ref int edited, int ind)
            {
                ...
                pegi.Edit_Enum(ref Value);

                if (Icon.Enter.Click())
                    edited = ind;
            }

            public override string ToString() => "{0} {1}".F(base.ToString(), pegi.GetNameForInspector(Value));
