    ...
    [Serializable] public class AbilityScores : SerializableDictionary_ForEnum<AbilityScore, CoreStat> 
    {
        ...
        protected override void InspectElementInList(AbilityScore key, int index)
        {
            ...
            if (value == null)
            {
                ...
                if ("{0} (Default)".F(name).PegiLabel(120).Edit_Delayed(ref current) && current > 0)
            ...
            Icon.Dice.Click(()=> 
                pegi.GameView.ShowNotification(Creature.inspectedCreature.SavingThrow(key, influence: RollInfluence.None).ToString()));
